...
- Added `test/test_agg_contract.c`: 210 unary type/operation contracts with
  grouped-versus-scalar value comparisons, 245 mixed numeric-pair contracts,
  and 336 registry/admission cases including buffered and parameterized ops.
  Registry presence is tested separately from engine admission.
- Added calling-thread dispatch counters, pure admission-reason inspection,
  and optional profiler ticks in `agg_engine.h/c`. Tests assert actual serial
  dense/hash and parallel dense/radix/hash dispatch, plus the legacy TIME-min
  fallback. Nullable-key and dense worker-budget decisions are inspectable.
- Fixed grouped BOOL/F32 median and quantile: native-width reads, F32 null
  checks, empty-group handling, and atomic null metadata for parallel output.
  Added slice, null-only, empty, filtered, mixed-aggregate and parallel tests.
...
- 13,916,401 input rows; 1,104,673 result groups.
- Warm full-table query approximately 120–140 ms; warm optimizer approximately
  0.003 ms. CSV loading was excluded from query measurements.
- An instrumented 8-core full-table run spent 40–45 ms in legacy radix record
  construction/scatter, 44–46 ms in partition hash/aggregation, and 2–3 ms in
  output emission. Remaining time was outside those instrumented phases.
- Alternating allocator-before/after builds on the extracted two columns had
  overlapping timings and no consistent improvement. These are local diagnostic
  measurements, not performance acceptance thresholds.
- `agg_resolve(OP_MIN, RAY_TIME)` returns NULL, so v2 admission fails. The
  legacy dense path declines nullable client_order_id; the legacy direct-insert
  path declines MIN. General legacy radix handles the query.
...
K1 is necessary for the GrandU fix: merely registering MIN(TIME) can move the
...
Covers K1/K3/K4. Depends on P1; combine with P2 for GrandU acceptance.
...
- Benchmark dense vs radix using actual range/cardinality and worker count.
  Do not solve GrandU by an unconditional dispatch override.
...
Acceptance: GrandU MIN(TIME) uses an efficient null-aware route and improves
...
- GrandU gains a measured improvement from an efficient nullable-key temporal
